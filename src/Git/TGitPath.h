...
public:
	...
	int FillUnRev(unsigned int Action, const CTGitPathList* list = nullptr, CString* err = nullptr);
	int FillBasedOnIndexFlags(unsigned short flag, unsigned short flagextended, const CTGitPathList* list = nullptr);
	int GetAction();
	/**
